    $('#firstbutton').mouseover(function() {
      document.getElementById('musickettei').currentTime = 0; //連続クリックに対応
      document.getElementById('musickettei').play(); //クリックしたら音を再生
    });
            ...
            { 
              ...
              let p_janken = ['グー','チョキ','パー'];//"プレイヤーのjanken"のリストを作成します。
                  ...
                  if (count===0){
                    // $('#main').append('<img id="badimg" src="img/badend.png" alt="">');
                    Result_end = "残念...やりなおしてね";
                    document.getElementById("gu").disabled = true;
                    document.getElementById("pa").disabled = true;
                    document.getElementById("cho").disabled = true;
                  }

                  if (sum1===3){ 
                    $('#main').append('<img id="clearimg" src="img/clear.jpg" alt="">');
                    Result_end = "三連勝おめでとう！！";
                  };
